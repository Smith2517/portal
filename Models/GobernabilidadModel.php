...
class GobernabilidadModel extends Mysql
{
	private $intItemId;
	...
	public function insertItem(string $nombre, string $descripcion, int $orden, int $estado, int $idpersona)
	{
		...
		$sql = "INSERT INTO gobernabilidad_items (nombre, descripcion, orden, estado, idpersona) VALUES (?, ?, ?, ?, ?)";
	...
	{
		$this->intItemId = $id;
		$sql = "SELECT * FROM gobernabilidad_items WHERE id = $this->intItemId";
	...
	public function updateItem(int $id, string $nombre, string $descripcion, int $orden, int $estado)
	{
		...
		$sql = "UPDATE gobernabilidad_items SET nombre = ?, descripcion = ?, orden = ?, estado = ?, fecha_actualizacion = NOW() WHERE id = $this->intItemId";
	...
	{
		$this->intItemId = $id;
		$sql = "DELETE FROM gobernabilidad_items WHERE id = $this->intItemId";
	...
	public function insertIndicador(int $itemId, string $nombre, string $descripcion, int $orden, int $estado, int $idpersona)
	{
		...
		$sql = "INSERT INTO gobernabilidad_indicadores (item_id, nombre, descripcion, orden, estado, idpersona) VALUES (?, ?, ?, ?, ?, ?)";
	...
	public function selectIndicadoresByItem(int $itemId)
	{
		$this->intItemId = $itemId;
		$sql = "SELECT gi.*, p.nombres, p.apellidos 
				FROM gobernabilidad_indicadores gi 
				INNER JOIN persona p ON gi.idpersona = p.idpersona 
	...
	public function selectIndicador(int $id)
	{
		...
		$sql = "SELECT * FROM gobernabilidad_indicadores WHERE id = $this->intIndicadorId";
		$request = $this->select($sql);
		return $request;
	}
	...
	public function updateIndicador(int $id, int $itemId, string $nombre, string $descripcion, int $orden, int $estado)
	{
		...
		$sql = "UPDATE gobernabilidad_indicadores SET item_id = ?, nombre = ?, descripcion = ?, orden = ?, estado = ?, fecha_actualizacion = NOW() WHERE id = $this->intIndicadorId";
	...
	{
		$this->intIndicadorId = $id;
		$sql = "DELETE FROM gobernabilidad_indicadores WHERE id = $this->intIndicadorId";
		$request = $this->delete($sql);
		return $request;
	}
	...
	public function insertArchivo(int $indicadorId, string $titulo, string $descripcion, string $archivoRuta, int $orden, int $estado, int $idpersona)
	{
		...
		$sql = "INSERT INTO gobernabilidad_archivos (indicador_id, titulo, descripcion, archivo_ruta, orden, estado, idpersona) VALUES (?, ?, ?, ?, ?, ?, ?)";
	...
	public function selectArchivosByIndicador(int $indicadorId)
	{
		...
		$sql = "SELECT ga.*, p.nombres, p.apellidos 
				FROM gobernabilidad_archivos ga 
	...
	public function selectArchivo(int $id)
	{
		...
		$sql = "SELECT * FROM gobernabilidad_archivos WHERE id = $this->intArchivoId";
		$request = $this->select($sql);
		return $request;
	}
	...
	public function updateArchivo(int $id, int $indicadorId, string $titulo, string $descripcion, string $archivoRuta, int $orden, int $estado)
	{
		...
		$sql = "UPDATE gobernabilidad_archivos SET indicador_id = ?, titulo = ?, descripcion = ?, archivo_ruta = ?, orden = ?, estado = ?, fecha_actualizacion = NOW() WHERE id = $this->intArchivoId";
		
		if ($archivoRuta != '') {
			$arrData = array($this->intIndicadorId, $this->strTitulo, $this->strDescripcion, $this->strArchivoRuta, $this->intOrden, $this->intEstado);
		} else {
			$sql = "UPDATE gobernabilidad_archivos SET indicador_id = ?, titulo = ?, descripcion = ?, orden = ?, estado = ?, fecha_actualizacion = NOW() WHERE id = $this->intArchivoId";
	...
	{
		$this->intArchivoId = $id;
		$sql = "DELETE FROM gobernabilidad_archivos WHERE id = $this->intArchivoId";
		$request = $this->delete($sql);
		return $request;
	}
	...
	public function getEstructuraGobernabilidad()
	{
		$sql = "SELECT 
				...
				FROM gobernabilidad_items gi
				LEFT JOIN gobernabilidad_indicadores gin ON gi.id = gin.item_id AND gin.estado=1
				LEFT JOIN gobernabilidad_archivos ga ON gin.id = ga.indicador_id AND ga.estado=1
				WHERE gi.estado = 1
