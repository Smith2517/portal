            ...
            <div class="row g-3">
                ...
                <div class="col-md-3">
                    <select class="form-select" id="filtroCategoria">
                        ...
                        <?php
                        $categorias = array();
                        if (isset($data['page_documentos']) && is_array($data['page_documentos'])) {
                            foreach ($data['page_documentos'] as $doc) {
                                if (!empty($doc['mn_categoria']) && !in_array($doc['mn_categoria'], $categorias)) {
                                    $categorias[] = $doc['mn_categoria'];
                                }
                            }
                        }
                ...
                <div class="col-md-3">
                    <select class="form-select" id="filtroAnio">
                        ...
                        if (isset($data['page_documentos']) && is_array($data['page_documentos'])) {
                            foreach ($data['page_documentos'] as $doc) {
                                if (!empty($doc['mn_fecha'])) {
                                    $anio = date('Y', strtotime($doc['mn_fecha']));
    ...
    <section class="py-4">
        <div class="container">
            <div class="row" id="listaDocumentos">
                ...
                <?php if (isset($data['page_documentos']) && count($data['page_documentos']) > 0): ?>
                    <?php foreach ($data['page_documentos'] as $value): ?>

                        <div class="col-12 mb-3 documento-item" 
                             data-categoria="<?= $value['mn_categoria'] ?>" 
                             data-anio="<?= !empty($value['mn_fecha']) ? date('Y', strtotime($value['mn_fecha'])) : '' ?>"
                             data-titulo="<?= strtolower($value['mn_titulo'] . ' ' . $value['mn_numero']) ?>">
                                    ...
                                    <div class="row align-items-center">
                                        ...
                                        <div class="col-md-7">
                                            <h5 class="card-title mb-1 text-primary">
                                                <?= $value['mn_titulo'] ?>
                                                <?php if (!empty($value['mn_numero'])): ?>
                                                    <small class="text-muted">(<?= $value['mn_numero'] ?>)</small>
                                                <?php endif; ?>
                                            </h5>
                                            
                                            <?php if (!empty($value['mn_categoria'])): ?>
                                                <span class="badge bg-secondary me-2">
                                                    <i class="fas fa-tag"></i> <?= $value['mn_categoria'] ?>
                                                </span>
                                            <?php endif; ?>
                                            
                                            <?php if (!empty($value['mn_fecha'])): ?>
                                                <span class="badge bg-info">
                                                    <i class="fas fa-calendar"></i> <?= date('d/m/Y', strtotime($value['mn_fecha'])) ?>
                                                </span>
                                            <?php endif; ?>
                                            
                                            <?php if (!empty($value['mn_descripcion'])): ?>
                                                <p class="card-text mt-2 text-muted small">
                                                    <?= limitar_cadena($value['mn_descripcion'], 150, '...') ?>
                                                </p>
                                            <?php endif; ?>
                                        </div>
                                        <div class="col-md-4 text-md-end mt-3 mt-md-0">
                                            <a href="<?= base_url() ?>/Assets/upload/documentos/<?= $value['mn_archivo'] ?>" 
                                               target="_blank" 
                                            ...
                                            <a href="<?= base_url() ?>/Assets/upload/documentos/<?= $value['mn_archivo'] ?>" 
                                               download="<?= $value['mn_archivo_original'] ?>" 
                ...
                <?php else: ?>
                    <div class="col-12">
                        <div class="alert alert-warning text-center">
                            <i class="fas fa-exclamation-triangle"></i> No hay documentos normativos registrados.
                        </div>
                    </div>
                <?php endif; ?>
