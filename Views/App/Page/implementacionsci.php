            ...
            <div class="row g-3">
                    ...
                    <select class="form-select" id="filtroCategoria">
                        ...
                        if (isset($arrDocumentos) && is_array($arrDocumentos)) {
                            foreach ($arrDocumentos as $doc) {
                                if (!empty($doc['is_categoria']) && !in_array($doc['is_categoria'], $categorias)) {
                                    $categorias[] = $doc['is_categoria'];
                                }
                            }
                        }
                ...
                <div class="col-md-3">
                    <select class="form-select" id="filtroAnio">
                        ...
                        if (isset($arrDocumentos) && is_array($arrDocumentos)) {
                            foreach ($arrDocumentos as $doc) {
                                if (!empty($doc['is_fecha'])) {
                                    $anio = date('Y', strtotime($doc['is_fecha']));
                ...
                <?php if (isset($arrDocumentos) && count($arrDocumentos) > 0): ?>
                    <?php foreach ($arrDocumentos as $value): ?>
                        ...
                        <div class="col-12 mb-3 documento-item"
                             ...
                             data-anio="<?= !empty($value['is_fecha']) ? date('Y', strtotime($value['is_fecha'])) : '' ?>"
                             data-titulo="<?= strtolower($value['is_titulo'] . ' ' . $value['is_numero']) ?>">
                                    ...
                                    <div class="row align-items-center">
                                        ...
                                        <div class="col-md-7">
                                            <h5 class="card-title mb-1 text-primary">
                                                <?= $value['is_titulo'] ?>
                                                <?php if (!empty($value['is_numero'])): ?>
                                                    <small class="text-muted">(<?= $value['is_numero'] ?>)</small>
                                                <?php endif; ?>
                                            </h5>

                                            <?php if (!empty($value['is_categoria'])): ?>
                                                <span class="badge bg-secondary me-2">
                                                    <i class="fas fa-tag"></i> <?= $value['is_categoria'] ?>
                                                </span>
                                            <?php endif; ?>

                                            <?php if (!empty($value['is_fecha'])): ?>
                                                <span class="badge bg-info">
                                                    <i class="fas fa-calendar"></i> <?= date('d/m/Y', strtotime($value['is_fecha'])) ?>
                                                </span>
                                            <?php endif; ?>

                                            <?php if (!empty($value['is_descripcion'])): ?>
                                                <p class="card-text mt-2 text-muted small">
                                                    <?= limitar_cadena($value['is_descripcion'], 150, '...') ?>
                                                </p>
                                            <?php endif; ?>
                                        </div>
                                        <div class="col-md-4 text-md-end mt-3 mt-md-0">
                                            <a href="<?= base_url() ?>/Assets/upload/documentos/<?= $value['is_archivo'] ?>"
                                               target="_blank"
                                            ...
                                            <a href="<?= base_url() ?>/Assets/upload/documentos/<?= $value['is_archivo'] ?>"
                                               download="<?= $value['is_archivo_original'] ?>"
                ...
                <?php else: ?>
                    <div class="col-12">
                        <div class="alert alert-warning text-center">
                            <i class="fas fa-exclamation-triangle"></i> No hay documentos de implementación registrados.
                        </div>
                    </div>
                <?php endif; ?>
