<?php
class Implementacionsci extends Controllers
{
    ...
    public function implementacionsci()
    {
        ...
        $data['page_name'] = "Implementación SCI";
        $data['page_functions_js'] = "functions_implementacionsci.js";
        $this->views->getView($this, "implementacionsci", $data);
    }

    public function getImplementacionsci()
    {
        if ($_SESSION['permisosMod']['r']) {
            $btnEdit = '';
            $btnDelete = '';
            $request = $this->model->selectImplementacionsci();
            $cont = 1;
            foreach ($request as $key => $value) {
                if ($value["is_estado"] == 1) {
                    ...
                    data-id="' . $value["id"] . '"
                    data-titulo="' . $value["is_titulo"] . '"
                    data-estado="' . (($value["is_estado"] == 1) ? 0 : 1) . '" ><i class="fa fa-toggle-on"></i></button>';
                    ...
                    data-id="' . $value["id"] . '"
                    data-titulo="' . $value["is_titulo"] . '"
                    data-estado="' . (($value["is_estado"] == 1) ? 0 : 1) . '"
                    ><i class="fa fa-toggle-off"></i></button>';
                }
                $request[$key]["is_estado"] = $status;

                $archivoBtn = '<a href="' . base_url() . '/Assets/upload/documentos/' . $value["is_archivo"] . '" target="_blank" class="btn btn-info btn-sm" title="Ver documento"><i class="fas fa-file-pdf"></i></a>';

                if ($_SESSION['permisosMod']['u']) {
                    $btnEdit = $btnEstado . '
                    <button class="btn btn-primary btn-sm btnEditFile"
                    data-id="' . $value["id"] . '"
                    data-file="' . $value["is_archivo"] . '"
                    title="Editar archivo"><i class="fa fa-file-pdf-o" aria-hidden="true"></i></button>
                    <button class="btn btn-primary btn-sm btnEdit"
                    data-id="' . $value["id"] . '"
                    data-titulo="' . $value["is_titulo"] . '"
                    data-numero="' . $value["is_numero"] . '"
                    data-fecha="' . $value["is_fecha"] . '"
                    data-categoria="' . $value["is_categoria"] . '"
                    data-descripcion="' . str_replace('"', '«', $value["is_descripcion"]) . '"
                    title="Editar"><i class="fas fa-pencil-alt"></i></button>';
                }
                if ($_SESSION['permisosMod']['d']) {
                    $btnDelete = '<button class="btn btn-danger btn-sm btnDel" data-id="' . $value["id"] . '" data-file="' . $value["is_archivo"] . '" data-titulo="' . $value["is_titulo"] . '"  title="Eliminar"><i class="far fa-trash-alt"></i></button>';
                }
    ...
    public function saveImplementacionsci()
    {
        if ($_SESSION['permisosMod']['w']) {
            ...
            $request = $this->model->insertImplementacionsci(
                $idPersona,
                $txtTitulo,
                $txtNumero,
                $txtFecha,
    ...
    public function updateImplementacionsci()
    {
        if ($_SESSION['permisosMod']['u']) {
            ...
            $request = $this->model->updateImplementacionsci(
                $idPersona,
                $txtTitulo,
                $txtNumero,
                $txtFecha,
    ...
    public function updateFileImplementacionsci()
    {
        if ($_SESSION['permisosMod']['u']) {
            if (!$_POST) {
                $arrResponse = array('status' => false, 'msg' => 'Método no encontrado');
            ...
            $request = $this->model->updateFileImplementacionsci($id, $file, $fileName);
            if ($request) {
    ...
    public function updateEstadoImplementacionsci()
    {
        if ($_SESSION['permisosMod']['u']) {
            if (!$_POST) {
                $arrResponse = array('status' => false, 'msg' => 'Método no encontrado');
            ...
            $request = $this->model->updateEstadoImplementacionsci($id, $estado);
            if ($request) {
    ...
    public function deleteImplementacionsci()
    {
        if ($_SESSION['permisosMod']['d']) {
            if (!$_POST) {
                $arrResponse = array('status' => false, 'msg' => 'Método no encontrado');
            ...
            $request = $this->model->deleteImplementacionsci($id);
            if ($request) {
