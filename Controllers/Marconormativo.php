<?php
class Marconormativo extends Controllers
{
    ...
    public function marconormativo()
    {
        ...
        $data['page_name'] = "Marco Normativo";
    ...
    public function getMarconormativo()
    {
        if ($_SESSION['permisosMod']['r']) {
            $btnEdit = '';
            $btnDelete = '';
            $request = $this->model->selectMarconormativo();
            $cont = 1;
            foreach ($request as $key => $value) {
                if ($value["mn_estado"] == 1) {
                    $status = '<span class="badge badge-primary">Publicado</span>';
                    $btnEstado = '<button class="btn btn-primary btn-sm btnEstado"
                    data-id="' . $value["id"] . '"
                    data-titulo="' . $value["mn_titulo"] . '"
                    data-estado="' . (($value["mn_estado"] == 1) ? 0 : 1) . '" ><i class="fa fa-toggle-on"></i></button>';
                } else if ($value["mn_estado"] == 0) {
                    $status = '<span class="badge badge-danger">No publicado</span>';
                    $btnEstado = '<button class="btn btn-danger btn-sm btnEstado"
                    data-id="' . $value["id"] . '"
                    data-titulo="' . $value["mn_titulo"] . '"
                    data-estado="' . (($value["mn_estado"] == 1) ? 0 : 1) . '"
                    ><i class="fa fa-toggle-off"></i></button>';
                }
                $request[$key]["mn_estado"] = $status;
                
                $archivoBtn = '<a href="' . base_url() . '/Assets/upload/documentos/' . $value["mn_archivo"] . '" target="_blank" class="btn btn-info btn-sm" title="Ver documento"><i class="fas fa-file-pdf"></i></a>';
                
                if ($_SESSION['permisosMod']['u']) {
                    $btnEdit = $btnEstado . '
                    <button class="btn btn-primary btn-sm btnEditFile"
                    data-id="' . $value["id"] . '"
                    data-file="' . $value["mn_archivo"] . '"
                    title="Editar archivo"><i class="fa fa-file-pdf-o" aria-hidden="true"></i></button>
                    <button class="btn btn-primary btn-sm btnEdit"
                    data-id="' . $value["id"] . '"
                    data-titulo="' . $value["mn_titulo"] . '"
                    data-numero="' . $value["mn_numero"] . '"
                    data-fecha="' . $value["mn_fecha"] . '"
                    data-categoria="' . $value["mn_categoria"] . '"
                    data-descripcion="' . str_replace('"', '«', $value["mn_descripcion"]) . '"
                    title="Editar"><i class="fas fa-pencil-alt"></i></button>';
                }
                if ($_SESSION['permisosMod']['d']) {
                    $btnDelete = '<button class="btn btn-danger btn-sm btnDel" data-id="' . $value["id"] . '" data-file="' . $value["mn_archivo"] . '" data-titulo="' . $value["mn_titulo"] . '"  title="Eliminar"><i class="far fa-trash-alt"></i></button>';
                }
    ...
    public function saveMarconormativo()
    {
        if ($_SESSION['permisosMod']['w']) {
            ...
            $request = $this->model->insertMarconormativo(
                $idPersona,
                $txtTitulo,
                $txtNumero,
                $txtFecha,
    ...
    public function updateMarconormativo()
    {
        if ($_SESSION['permisosMod']['u']) {
            ...
            $request = $this->model->updateMarconormativo(
                $idPersona,
                $txtTitulo,
                $txtNumero,
                $txtFecha,
    ...
    public function updateFileMarconormativo()
    {
        if ($_SESSION['permisosMod']['u']) {
            if (!$_POST) {
                $arrResponse = array('status' => false, 'msg' => 'Método no encontrado');
            ...
            $request = $this->model->updateFileMarconormativo($id, $file, $fileName);
            if ($request) {
    ...
    public function updateEstadoMarconormativo()
    {
        if ($_SESSION['permisosMod']['u']) {
            if (!$_POST) {
                $arrResponse = array('status' => false, 'msg' => 'Método no encontrado');
            ...
            $request = $this->model->updateEstadoMarconormativo($id, $estado);
            if ($request) {
    ...
    public function deleteMarconormativo()
    {
        if ($_SESSION['permisosMod']['d']) {
            if (!$_POST) {
                $arrResponse = array('status' => false, 'msg' => 'Método no encontrado');
            ...
            $request = $this->model->deleteMarconormativo($id);
            if ($request) {
