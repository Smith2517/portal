...
function loadTable() {
  table = $("#table").dataTable({
    ...
    ajax: {
      url: " " + base_url + "/Barrasnavegacion/getSection/" + idFooter,
      dataSrc: "",
    },
    columns: [
      { data: "cont" },
      { data: "sbn_titulo" },
      { data: "sbn_descripcion" },
      { data: "sbn_url" },
      { data: "sbn_estado" },
      { data: "acciones" },
    ],
  ...
  arrbtnEdit.forEach((element) => {
    element.addEventListener("click", () => {
      ...
      document.querySelector("#txtDescripcion_upd").value = element
        ...
        .replace(/«/g, '"');
      if (document.querySelector("#txtUrl_upd")) {
...
function save() {
  if (document.querySelector("#formSave")) {
    ...
    formSave.addEventListener("submit", (e) => {
      ...
      let url = base_url + "/Barrasnavegacion/saveSection";
      try {
        divLoading.style.display = "flex";
        fetch(url, config)
      ...
      swal(
        ...
        function (isConfirm) {
          if (isConfirm) {
            ...
            let url = base_url + "/Barrasnavegacion/deleteSections";
            try {
              divLoading.style.display = "flex";
              fetch(url, config)
                .then((response) => response.json())
...
function updat() {
  if (document.querySelector("#formUpdate")) {
    ...
    formUpdate.addEventListener("submit", (e) => {
      ...
      let url = base_url + "/Barrasnavegacion/updateSection";
      try {
        divLoading.style.display = "flex";
        fetch(url, config)
          .then((response) => response.json())
      ...
      swal(
        ...
        function (isConfirm) {
          if (isConfirm) {
            ...
            let url = base_url + "/Barrasnavegacion/updateEstadoSection";
            try {
              divLoading.style.display = "flex";
              fetch(url, config)
                .then((response) => response.json())
