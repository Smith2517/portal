...
const apiUrl = base_url + '/page/getCategoriasBlog';
...
function cargarElementos() {
    ...
    fetch(`${apiUrl}?_page=${paginaActual}&_limit=${elementosPorPagina}`)
        .then(response => {
            if (!response.ok) {
...
function mostrarElementosEnCards(elementos) {
    elementos.forEach(elemento => {
        ...
        cardTitle.textContent = elemento.c_Categoria;
        const cardText = document.createElement('p');
        cardText.className = 'card-text';
        cardText.textContent = elemento.c_Descripcion;
        const cardImage = document.createElement('img');
        cardImage.className = 'card-img-top'; // Agrega la clase si necesitas estilos específicos para la imagen
        cardImage.src = base_url + "/Assets/upload/images/" + elemento.c_Imagen; // Reemplaza 'URL_DE_LA_IMAGEN' con la URL de tu imagen
        cardImage.alt = elemento.c_Categoria; // Agrega una descripción para la imagen
        const cardLink = document.createElement('a');
        cardLink.className = 'btn';
        cardLink.textContent = "Ver más";
        cardLink.href = base_url + "/page/b/" + elemento.idCategoria;
