...
function loadTable() {
  table = $("#table").dataTable({
    ...
    ajax: {
      url: " " + base_url + "/Materialdidactico/getMaterialdidactico",
      dataSrc: "",
    },
    columns: [
      { data: "cont" },
...
function openModalSave() {
  if (document.querySelector("#openModal")) {
    document.querySelector("#openModal").addEventListener("click", () => {
      ...
      document.querySelector("#titleSave").innerHTML = "Agregar Nuevo Material";
  ...
  if (document.querySelectorAll(".btnEdit").length > 0) {
    document.querySelectorAll(".btnEdit").forEach((btn) => {
      btn.addEventListener("click", function () {
        document.querySelector("#titleEdit").innerHTML = "Editar Información del Material";
...
function openModalFileEdit() {
  if (document.querySelectorAll(".btnEditFile").length > 0) {
    document.querySelectorAll(".btnEditFile").forEach((btn) => {
      btn.addEventListener("click", function () {
        document.querySelector("#titleEditFile").innerHTML = "Reemplazar Archivo PDF";
...
function save() {
  if (document.querySelector("#formSave")) {
    ...
    formSave.addEventListener("submit", (e) => {
      ...
      let url = base_url + "/Materialdidactico/saveMaterialdidactico";
      try {
        if (divLoading) divLoading.style.display = "flex";
        fetch(url, config)
...
function update() {
  if (document.querySelector("#formUpdate")) {
    ...
    formUpdate.addEventListener("submit", (e) => {
      ...
      let url = base_url + "/Materialdidactico/updateMaterialdidactico";
      try {
        if (divLoading) divLoading.style.display = "flex";
        fetch(url, config)
          .then((response) => response.json())
...
function updateFile() {
  if (document.querySelector("#formUpdateFile")) {
    ...
    formUpdateFile.addEventListener("submit", (e) => {
      ...
      let url = base_url + "/Materialdidactico/updateFileMaterialdidactico";
      try {
        if (divLoading) divLoading.style.display = "flex";
        fetch(url, config)
          .then((response) => response.json())
  ...
  if (document.querySelectorAll(".btnEstado").length > 0) {
    document.querySelectorAll(".btnEstado").forEach((btn) => {
      btn.addEventListener("click", function () {
        ...
        swal({
          title: "¿Está seguro de " + texto + " el material: " + nombre + "?",
          text: "Esta acción cambiará el estado del material.",
          icon: "warning",
          buttons: {
            cancel: {
              text: "Cancelar",
          ...
          if (willDelete) {
            ...
            let url = base_url + "/Materialdidactico/updateEstadoMaterialdidactico";
            try {
              if (divLoading) divLoading.style.display = "flex";
              fetch(url, config)
                .then((response) => response.json())
  ...
  if (document.querySelectorAll(".btnDel").length > 0) {
    document.querySelectorAll(".btnDel").forEach((btn) => {
      btn.addEventListener("click", function () {
        ...
        let nombre = this.getAttribute("data-nombre");
        swal({
          title: "¿Está seguro de eliminar el material: " + nombre + "?",
          ...
          if (willDelete) {
            ...
            let url = base_url + "/Materialdidactico/deleteMaterialdidactico";
            try {
              if (divLoading) divLoading.style.display = "flex";
              fetch(url, config)
                .then((response) => response.json())
